...
#ifndef LIBIOP_RELATIONS_VARIABLE_TCC_
#define LIBIOP_RELATIONS_VARIABLE_TCC_
...
{
    this->terms.emplace_back(linear_term<FieldT>(var.index_, FieldT::one()));
}

template<typename FieldT>
void linear_combination<FieldT>::add_term(const variable<FieldT> &var, const integer_coeff_t int_coeff)
{
    this->terms.emplace_back(linear_term<FieldT>(var.index_, int_coeff));
}

template<typename FieldT>
void linear_combination<FieldT>::add_term(const variable<FieldT> &var, const FieldT &coeff)
{
    this->terms.emplace_back(linear_term<FieldT>(var.index_, coeff));
}
...
FieldT linear_combination<FieldT>::evaluate(const std::vector<FieldT> &assignment) const
{
    ...
    for (auto &lt : terms)
    {
        acc += (lt.index_ == 0 ? FieldT::one() : assignment[lt.index_-1]) * lt.coeff_;
    }
    return acc;
}
...
bool linear_combination<FieldT>::is_valid(const size_t num_variables) const
{
    ...
    {
        if (terms[i-1].index_ >= terms[i].index_)
        {
            return false;
        }
    }
    ...
    if ((--terms.end())->index_ >= num_variables)
    {
        return false;
    }
...
void linear_combination<FieldT>::print_with_assignment(const std::vector<FieldT> &full_assignment, const std::map<size_t, std::string> &variable_annotations) const
{
    for (auto &lt : terms)
    {
        ...
        {
            printf("    x_%zu * ", lt.index_);
            lt.coeff_.print();

            auto it = variable_annotations.find(lt.index_);
            printf("    where x_%zu (%s) was assigned value ", lt.index_,
                   (it == variable_annotations.end() ? "no annotation" : it->second.c_str()));
            full_assignment[lt.index_-1].print();
...
#endif // LIBIOP_RELATIONS_VARIABLE_TCC
